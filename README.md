...
| 操作 | 页面表现 |
| --- | --- |
| `/tree` 切换分支 | 页面**自动**整体重载为当前分支内容（地址不变） |
| `/fork`、`/clone`、`/new`、`/resume` | 页面自动切换到新会话的内容，标题与侧栏一并更新；**服务不重建、地址不变**，不用重新 `/live` |
| `/compact` | 页面自动刷新 |
| `/name` 改名 | 标签页标题与侧栏立即更新 |
| 退出 pi | 服务关闭、端口释放 |

实时状态存放在进程级共享对象里，因此 pi 在会话替换时重建扩展实例也不会让服务失联。
注意：服务是**跟随当前会话**的，所以所有已打开的页面会一起切到新会话；
如果想同时观察两个会话，请开两个 pi 实例（各自 `/live`，端口互不影响）。
